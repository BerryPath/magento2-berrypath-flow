...
class Preview extends Action implements HttpGetActionInterface
{
    ...
    public function execute(): Raw
    {
        ...
        if (!$this->canGenerate($storeId)) {
            return $this->rawResponse((string)__('BerryPath feed is not available.'), 'text/plain');
        }
    ...
    {
        $xml = $this->formatter->toXml($feed);
        $notice = $isLimited
            ? (string)__('Preview limited to the first %1 products. The XML feed exports all products.', self::PREVIEW_PRODUCT_LIMIT)
            : '';

        return '<!doctype html><html><head><meta charset="utf-8"><title>BerryPath Feed Preview</title>'
